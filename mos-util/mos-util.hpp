...
template<typename T>
constexpr volatile T& ram(const uint16 address) {
    return *reinterpret_cast<T*>(address);
}

volatile byte& ram8(const uint16 address) {
    return *reinterpret_cast<byte*>(address);
}

volatile uint16& ram16(const uint16 address) {
    return *reinterpret_cast<uint16*>(address);
}

volatile uint32& ram32(const uint16 address) {
